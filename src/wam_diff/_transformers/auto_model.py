...
import wam_diff.components.distributed.utils as dist_utils
from wam_diff import __version__
from wam_diff._transformers.registry import ModelRegistry
from wam_diff.components.distributed.init_utils import (
    get_local_world_size_preinit,
    get_world_size_safe,
)
from wam_diff.components.utils.model_utils import resolve_trust_remote_code
from wam_diff.shared.import_utils import safe_import
from wam_diff.shared.utils import dtype_from_str
...
def _patch_attention(obj, sdpa_method=None):
    """
    Wrap the `forward` method of `obj` in an `sdpa_kernel` context manager.
...
def _patch_liger_kernel(model):
    ...
    try:
        ...
        if model_type in ["qwen3_vl", "qwen3_vl_uniform"]: # 自定义模型用本地的apply方法
            apply_fn = MODEL_TYPE_TO_APPLY_LIGER_FN[model_type]
            apply_fn(model=model)
            logging.info("Applied liger-kernel to local model")
        else:
            liger_kernel_trf._apply_liger_kernel_to_instance(model=model)
            logging.info("Applied liger-kernel to transformer model")
        return model
    except Exception:
...
def apply_liger_kernel_to_qwen3_vl(
    ...
    """
    Apply Liger kernels to replace original implementation in local Qwen3-VL models.
...
MODEL_TYPE_TO_APPLY_LIGER_FN = {
    ...
    "qwen3_vl_uniform": apply_liger_kernel_to_qwen3_vl,
}
...
def _download_model_weights(hf_config, pretrained_model_name_or_path):
    if not os.path.isdir(pretrained_model_name_or_path):
        ...
        # `wam_diff.components.distributed.utils.FirstRankPerNode`.
