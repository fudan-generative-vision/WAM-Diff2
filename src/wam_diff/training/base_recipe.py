...
from wam_diff._transformers.auto_tokenizer import NeMoAutoTokenizer
from wam_diff.components.checkpoint.checkpointing import save_config
from wam_diff.components.config.loader import ConfigNode
from wam_diff.components.optim.scheduler import OptimizerParamScheduler
from wam_diff.components.training.rng import StatefulRNG
...
class BaseRecipe:
    ...
    def save_checkpoint(
        ...
        if is_dist_initialized:
            ...
            # Remote checkpoint sync is explicitly opt-in. Public runs never
            # contact a cloud endpoint unless WAM_DIFF_REMOTE_CHECKPOINT_URI is set.
            output_url = os.getenv("WAM_DIFF_REMOTE_CHECKPOINT_URI")
            current_rank = dist.get_rank() if dist.is_initialized() else 0
            if output_url and current_rank % 8 == 0:
                try:
                    import moxing as mox
                except ImportError as exc:
                    raise RuntimeError(
                        "Remote checkpoint sync requires the optional moxing package."
                    ) from exc

                basename = os.path.basename(path.rstrip("/"))
                dst_path = f"{output_url.rstrip('/')}/{basename}/rank_{current_rank}/"
    ...
    def _update_best_symlink(self, target_dir: str, val_loss: float) -> None:
        ...
        if val_loss < self.__dict__.get("_best_val_loss", float("inf")):
            self.__dict__["_best_val_loss"] = val_loss
    ...
    def _log_library_versions(self):
        """Log import paths and versions for wam_diff, transformers, and torch."""
        if not getattr(self, "dist_env", None) or not getattr(self.dist_env, "is_main", False):
            return
        try:
            import wam_diff as nemo_am
        ...
        try:
            ...
            tfm_path = "<unknown>"
        libs = {
            "wam_diff": {"version": getattr(nemo_am, "__version__", None), "import_path": nemo_path},
