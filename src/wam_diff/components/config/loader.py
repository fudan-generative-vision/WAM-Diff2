...
ALLOWED_IMPORT_PREFIXES = ("wam_diff", "torch", "transformers", "torchdata", "torchao", "liger_kernel")
...
ENABLE_USER_MODULES = os.environ.get(
    "WAM_DIFF_ENABLE_USER_MODULES", os.environ.get("NEMO_ENABLE_USER_MODULES", "")
).lower() in ("1", "true", "yes")
...
def set_enable_user_modules(allow: bool) -> None:
    ...
    Users can also set WAM_DIFF_ENABLE_USER_MODULES=1. The former
    NEMO_ENABLE_USER_MODULES name remains accepted for compatibility.
    """
    ...
    if ":" in dotted_path:
        ...
        if not _is_safe_attr(attr):
            raise ImportError(
                ...
                "To allow out-of-tree code, set WAM_DIFF_ENABLE_USER_MODULES=1 or call set_enable_user_modules(True)."
            )
