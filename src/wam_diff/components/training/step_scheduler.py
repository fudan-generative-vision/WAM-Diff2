...
from wam_diff.components.training.signal_handler import DistributedSignalHandler
...
class StepScheduler(Stateful):
    ...
    def __init__(
        ...
        try:
            self.epoch_len = ceil(len(dataloader) / self.grad_acc_steps) # 这里的dataloader已经是分到每个GPU上的
        except:
            self.epoch_len = None
    ...
    def is_ckpt_step(self):
        ...
        return is_ckpt_step or self.is_last_step or self.sigterm_received
